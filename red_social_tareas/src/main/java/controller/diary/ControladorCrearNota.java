...
public class ControladorCrearNota extends ControladorFuncionesCompartidas {
    ...
    public void setListener(NotaCreadaListener listener) {
    ...
    public void initialize() {
        ...
        btnCompartir.setOnAction(e -> seleccionarEstado(visibilidadSeleccionada = VisibilidadNota.COMPARTIR));
        btnPublico.setOnAction(e -> seleccionarEstado(visibilidadSeleccionada = VisibilidadNota.PUBLICO));
        btnPrivado.setOnAction(e -> seleccionarEstado(visibilidadSeleccionada = VisibilidadNota.PRIVADO));

        //validar fechas solo de la actual a adelante
    ...
    private void seleccionarEstado(VisibilidadNota estado) {
        this.visibilidadSeleccionada = estado;

        marcarBotonActivo(btnCompartir, estado == VisibilidadNota.COMPARTIR);
        marcarBotonActivo(btnPublico, estado == VisibilidadNota.PUBLICO);
        marcarBotonActivo(btnPrivado, estado == VisibilidadNota.PRIVADO);

        peticionCrearNota();
    }

    private void marcarBotonActivo(Button btn, boolean activo) {
        btn.setOpacity(activo ? 1.0 : 0.85);
    }
        ...
        new Thread(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    ...
                    if (!(resp.data instanceof CrearNotaRespuestaDTO out)) {
    ...
    private ValidacionResultado validarFormulario() {
        ...
        if (inicioTxt.isEmpty()) {
            ...
            try {
                ...
                ok = false; sb.append("• Hora inicio inválida. Usa HH:mm (ej: 09:30).\n"); marcarError(txtHoraInicio);
            }
        }
        ...
        if (!finTxt.isEmpty()) {
            try {
                ...
                ok = false; sb.append("• Hora fin inválida. Usa HH:mm (ej: 18:00).\n"); marcarError(txtHoraFin);
            }
            if (inicio != null && fin != null && fin.isBefore(inicio)) {
                ok = false; sb.append("• La hora fin no puede ser anterior a la hora inicio.\n"); marcarError(txtHoraFin);
            }
        } else quitarError(txtHoraFin);

        if (txtTexto.getText() == null || txtTexto.getText().trim().isEmpty()) {
            ok = false; sb.append("• El texto no puede estar vacío.\n"); marcarError(txtTexto);
        } else quitarError(txtTexto);

        if (visibilidadSeleccionada == null) {
            ok = false; sb.append("• Selecciona un estado de publicación.\n");
        }
